...
developers = {
    "AvizonRP - Anonymous Sry XD", -- Main Developer
    "AvizonRP - Anonymous Sry XD", -- Secondary Developer
    "AvizonRP - Anonymous Sry XD" -- Tertiary Developer and GitHub Maintenance
}

menuName = "AvizonRP - SkidMenu" -- The name of the menu
version = "1.2" -- Keep it simple
...
menuKeybind = "," -- Key to open the menu. Supported ones are shown below (line 1287) - Find new ones at https://docs.fivem.net/game-references/controls/
noclipKeybind = "F7" -- Key to toggle Noclip
startMessage = "∑ ~b~Siemano kurwa, " .. GetPlayerName(PlayerId()) .. "." -- The message that is shown when the menu is opened
subMessage = "~w~Wcisnij ~b~" .. menuKeybind .. "~w~ by otworzyc menu." -- subtitle of opening message
motd = "∑ Używasz wersji z ~r~04.11.2019r, z godziny 17:10 ~w~:D !" -- motd
...
Citizen.CreateThread(function()
	...
	WarMenu.CreateSubMenu('custom', 'vehiclespawner', 'custom')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('skid') then
            ...
            elseif WarMenu.Button('~r~Wylacz Calkowicie Menu') then break
            end
        ...
        elseif WarMenu.IsMenuOpened('vehiclespawner') then
			...
			elseif WarMenu.MenuButton('Pojazdy AvizonRP', 'custom') then
			end
		...
		elseif WarMenu.IsMenuOpened('custom') then
            for i = 1, #custom do
                local vehname = GetLabelText(custom[i])
                if vehname == "NULL" then vehname = custom[i] end
                local carButton = WarMenu.Button(vehname)
                if carButton then
                    SpawnVeh(custom[i], PlaceSelf, SpawnEngineOn)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('lua') then
            if WarMenu.MenuButton('AvizonRP Options', 'esx') then
        ...
        elseif WarMenu.IsMenuOpened('esx') then
            if WarMenu.Button("~r~AvizonRP ~w~Zarcie / Picie") then
                TriggerEvent('esx_status:seet', 'hunger', 1000000)
				TriggerEvent('esx_status:seet', 'thirst', 1000000)
			elseif WarMenu.Button("~r~AvizonRP ~w~Przebierz Sie") then
				TriggerEvent('esx_skin:openSaveableMenu')
				WarMenu.CloseMenu()	
            elseif WarMenu.Button("~r~AvizonRP ~w~Off Duty / On duty") then
                TriggerServerEvent('duty:ambulance')
				TriggerServerEvent('duty:police')
				TriggerServerEvent('duty:mecano')
            elseif WarMenu.Button("~r~AvizonRP ~w~Wskrzesz Sie") then
                TriggerServerEvent('esx_ambulancejob:revivee', GetPlayerServerId(PlayerId()))
            elseif WarMenu.Button("~r~AvizonRP ~w~Wskrzesz Przez ID") then
                local serverID = GetKeyboardInput("Enter Player Server ID:")
                TriggerServerEvent('esx_ambulancejob:revivee', serverID)
			elseif WarMenu.Button("~r~AvizonRP ~w~Dodaj Wszystkie Licencje") then
				...
				TriggerServerEvent('esx_dmvschool:addLicense', 'boating')
				TriggerServerEvent('esx_dmvschool:addLicense', 'aircraft')
			elseif WarMenu.Button("~r~AvizonRP ~w~Skroc Wyrok (Wiezienie~r~(Nie Testowane)~w~)") then
				local JailTime = 0
				local fastTimer = 0
				local minusTime = math.random(10,100)
				JailTime = JailTime - minusTime
				TriggerServerEvent('esx_jailer:updateRemaining', JailTime)
				fastTimer = fastTimer - minusTime
			elseif WarMenu.Button("~r~AvizonRP ~w~Zrzuć Worek Z Głowy") then
				TriggerServerEvent('esx_worek:zdejmij')	
			elseif WarMenu.Button("~r~AvizonRP ~w~Otworz Pojazd") then
				local playerPed = PlayerPedId()
				local vehicle   = ESX.Game.GetVehicleInDirection()
				local coords    = GetEntityCoords(playerPed)

				if IsPedSittingInAnyVehicle(playerPed) then
					ESX.ShowNotification('Jestes juz w pojezdzie ty kurwo jebana xD!!!')
				end
				...
				if DoesEntityExist(vehicle) then
					...
					Citizen.CreateThread(function()
						...
						ESX.ShowNotification('Pojazd Otwarty Brawo Kurwiszonie XD!!!')
						isBusy = false
					end)
				else
					ESX.ShowNotification('Nie ma tutaj pierdolonego pojazdu!!!')
				end
			elseif WarMenu.Button("~r~AvizonRP ~w~Zeruj Licznik Pojazdu") then
				...
				if IsPedInVehicle(ped, car, false) then
					ESX.ShowAdvancedNotification('Komputer', 'Reset przebiegu', "~y~Licznik został zresetowany w aucie o numerach: ~g~"..plate, pic, 1)
					TriggerServerEvent('esx_reset:zeruj',plate)
				else
					ESX.ShowAdvancedNotification('Komputer', 'Reset przebiegu', "~r~Musisz siedzieć w aucie, aby zresetować licznik!", pic, 1)
				end
			elseif WarMenu.Button("~r~AvizonRP ~w~Kajdanki (~g~Nawet Jak Nie Posiadasz~w~)") then
				TriggerEvent('tostKajdany:MenuKajdanek')
				WarMenu.CloseMenu()	
			elseif WarMenu.Button("~r~AvizonRP ~w~Telefon (~g~Nawet Jak Nie Posiadasz~w~)") then
				TriggerEvent('gcPhone:forceOpenPhone')
				WarMenu.CloseMenu()	
			elseif WarMenu.Button("~r~AvizonRP ~w~Narkotyki Zbieranie") then
			...
			elseif WarMenu.Button("~r~AvizonRP ~w~Narkotyki Przerabianie") then
			...
			elseif WarMenu.Button("~r~AvizonRP ~w~Narkotyki Stop") then
			...
			elseif WarMenu.Button("~r~AvizonRP ~w~Zbuguj/Odbuguj Gracza Przez ID") then
                local serverID = GetKeyboardInput("Enter Player Server ID:")
				TriggerServerEvent('esx_gang1job:handcuff', serverID)
				TriggerServerEvent('esx_gang2job:handcuff', serverID)
				TriggerServerEvent('esx_gang3job:handcuff', serverID)
				TriggerServerEvent('esx_mafiajob:handcuff',serverID)
				TriggerServerEvent('esx_policejob:handcufff', serverID)
			elseif WarMenu.Button("~r~AvizonRP ~w~Daj Szmal ~g~+100 000$") then
                TriggerServerEvent('esx_ambulancejob:revivee', GetPlayerServerId(PlayerId()))
                TriggerServerEvent('esx_ambulancejob:revivee', GetPlayerServerId(PlayerId()))
				TriggerServerEvent('esx_ambulancejob:revivee', GetPlayerServerId(PlayerId()))
                TriggerServerEvent('esx_ambulancejob:revivee', GetPlayerServerId(PlayerId()))
